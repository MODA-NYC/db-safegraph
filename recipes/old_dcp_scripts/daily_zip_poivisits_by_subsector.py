...
SELECT safegraph_place_id, poi_cbg, postal_code, date_add('day', (row_number() over(partition by safegraph_place_id, date_start)) - 1, date_start) AS date_current, CAST(visits AS SMALLINT) as visits
FROM (
  SELECT
     safegraph_place_id,
     poi_cbg,
...
for year_qrtr, range in quarters.items():
    ...
    aws.execute_query(
        ...
        database="safegraph", 
        output=f"output/dev/poi/daily_zip_poivisits_by_subsector/daily_zip_poivisits_by_subsector_{year_qrtr}.csv.zip"
    )
