#####
#finds visitors, bucketed by late night, morning, afternoon, and evening, weekday and weekend.
#####
#imports
import datetime
...
def main():
    ...
    filename = 'naics_{}_{}_{}.csv'.format(YEAR, MONTH, DAY)
    
    filtered_current_places = filtered_current_places.drop([
...
class week_dto:
    ...
    def get_weekly_patterns_v2(self, nyc_counties, nrows):        
        ...
        print("part 5")
        part_5 = pd.read_csv(Path(self.weekly_patterns_path + '/' + 'patterns-part5.csv'), nrows=nrows)
        self.wp_df_raw.append(part_2)
        self.wp_df_raw.append(part_3)
        self.wp_df_raw.append(part_4)
        self.wp_df_raw.append(part_5)
...
def load_core_places(my_n_rows):
    ...
    print("part 1")
    df = pd.read_csv(CORE_PLACES_PATH / 'core_poi-part1.csv', nrows=my_n_rows)
    print("part 2")
    temp = pd.read_csv(CORE_PLACES_PATH / 'core_poi-part2.csv', nrows=my_n_rows)
    df = df.append(temp, ignore_index=True)
    print("part 3")
    temp = pd.read_csv(CORE_PLACES_PATH  / 'core_poi-part3.csv', nrows=my_n_rows)
    df = df.append(temp, ignore_index=True)
    print("part 4")
    temp = pd.read_csv(CORE_PLACES_PATH / 'core_poi-part4.csv', nrows=my_n_rows)
    df = df.append(temp, ignore_index=True)
    print("part 5")
    temp = pd.read_csv(CORE_PLACES_PATH / 'core_poi-part5.csv', nrows=my_n_rows)
    df = df.append(temp, ignore_index=True)
    print("part 6")
    temp = pd.read_csv(CORE_PLACES_PATH / 'core_poi-part6.csv', nrows=my_n_rows)
    df = df.append(temp, ignore_index=True)
    return df
